...
export function LifecycleSection({ isActive, identityType, scrollContainerRef, unlockScroll }: LifecycleSectionProps) {
  ...
  const buttonOpacity = useTransform(scrollYProgress, [0.70, 0.80], [0, 1])
  const buttonY = useTransform(scrollYProgress, [0.70, 0.80], [20, 0])

  // Exit Animation (Cover everything)
  // Happens at very end (0.95 -> 1.0)
  const exitProgress = useTransform(scrollYProgress, [0.95, 1.0], [1, 0])

  // Scroll Indicator Opacity
  // Fades out before first card appears (which starts at 0.10)
  const indicatorOpacity = useTransform(scrollYProgress, [0.02, 0.08], [1, 0])

  return (
    <section ref={containerRef} className="relative h-[450vh] w-full">
