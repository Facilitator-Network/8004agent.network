...
void main(){
  ...
  if (uHoverState == 1) {
    vec2 touchUV = gl_FragCoord.xy / uResolution;
    // Flip Y for texture lookup if needed, but usually Three.js textures match
    // Touch texture is drawn 0..1, here we mapped screen to it.
    // However, touch texture data is updated with touch.addTouch(fx/w, fy/h).
    // Let's sample uTouch directly.
    float hover = texture(uTouch, touchUV).r;
    feed += hover * uHoverStrength;
  }
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (!threeRef.current) mustReinit = true;
      ...
      if (prevConfigRef.current.liquid !== cfg.liquid || prevConfigRef.current.noiseAmount !== cfg.noiseAmount || prevConfigRef.current.antialias !== cfg.antialias) {
           mustReinit = true;
      }
      // Re-init if enabling hover for the first time when liquid was off
      if (enableHover && !threeRef.current.touch) mustReinit = true;
    }
    if (mustReinit) {
      if (threeRef.current) {
      ...
      const uniforms = {
        ...
        uFade: { value: edgeFade },
        uHoverState: { value: enableHover ? 1 : 0 },
        uHoverStrength: { value: hoverPixelStrength },
      ...
      t.uniforms.uFade.value = edgeFade;
      ...
      t.uniforms.uHoverState.value = enableHover ? 1 : 0;
      t.uniforms.uHoverStrength.value = hoverPixelStrength;
      // If we enabled hover but touch wasn't created, we need reinit which is handled above
      if (enableHover && t.touch) t.uniforms.uTouch.value = t.touch.texture;
    }
